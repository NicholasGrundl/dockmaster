...
<!-- diagram: Simplified illustration of the agent loop showing the cycle between user input, model inference, and tool calls -->
...
<!-- diagram: Multi-turn conversation showing how conversation history grows with each turn -->
...
The Responses API endpoint that the Codex CLI uses is [configurable](https://github.com/openai/codex/blob/main/codex-cli/README.md#configuration), so it can be used with any endpoint that implements the Responses API:

- When using [ChatGPT login](https://github.com/openai/codex/blob/main/codex-cli/README.md#chatgpt-login) with the Codex CLI, it uses `https://chatgpt.com/backend-api/codex/responses` as the endpoint
- When using [API-key authentication](https://github.com/openai/codex/blob/main/codex-cli/README.md#api-key) with OpenAI hosted models, it uses `https://api.openai.com/v1/responses` as the endpoint
- When running Codex CLI with `--oss` to use gpt-oss with [ollama 0.13.4+](https://ollama.com/) or [LM Studio 0.3.39+](https://lmstudio.ai/), it defaults to `http://localhost:11434/v1/responses` running locally on your computer
...
The [Responses API](https://platform.openai.com/docs/api-reference/responses) takes a JSON payload with many parameters. We'll focus on these three:

- [instructions](https://platform.openai.com/docs/api-reference/responses/create#responses-create-instructions): system (or developer) message inserted into the model's context
- [tools](https://platform.openai.com/docs/api-reference/responses/create#responses-create-tools): a list of tools the model may call while generating a response
- [input](https://platform.openai.com/docs/api-reference/responses/create#responses-create-input): a list of text, image, or file inputs to the model

In Codex, the `instructions` field is read from the [model_instructions_file](https://github.com/openai/codex/blob/main/codex-cli/README.md#configuration) in `~/.codex/config.toml`, if specified; otherwise, the [base_instructions associated with a model](https://github.com/openai/codex/tree/main/codex-cli/src/prompts) are used. Model-specific instructions live in the Codex repo and are bundled into the CLI (e.g., [gpt-5.2-codex_prompt.md](https://github.com/openai/codex/blob/main/codex-cli/src/prompts/gpt-5.2-codex_prompt.md)).
...
Finally, the `input` field of the JSON payload is a list of items. Codex inserts the following [items](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts) into the input before adding the user message:
...
The message is built from a template where the key pieces of content come from snippets of Markdown bundled into the Codex CLI, such as [workspace_write.md](https://github.com/openai/codex/blob/main/codex-cli/src/prompts/workspace_write.md) and [on_request.md](https://github.com/openai/codex/blob/main/codex-cli/src/prompts/on_request.md):
...
3. (Optional) A message with `role=user` whose contents are the "user instructions," which are not sourced from a single file but are [aggregated across multiple sources](https://github.com/openai/codex/blob/main/codex-cli/src/user-instructions.ts). In general, more specific instructions appear later:

- Contents of `AGENTS.override.md` and `AGENTS.md` in `$CODEX_HOME`
- Subject to a limit (32 KiB, by default), look in each folder from the Git/project root of the cwd (if it exists) up to the cwd itself: add the contents of any of `AGENTS.override.md`, `AGENTS.md`, or any filename specified by `project_doc_fallback_filenames` in `config.toml`
- If any [skills](https://github.com/openai/codex/blob/main/codex-cli/README.md#skills) have been configured:
  - a short preamble about skills
  - the [skill metadata](https://github.com/openai/codex/blob/main/codex-cli/src/skills.ts) for each skill
  - a section on [how to use skills](https://github.com/openai/codex/blob/main/codex-cli/src/prompts/skills_usage.md)

4. A message with `role=user` that describes the local environment in which the agent is currently operating. This specifies the [current working directory and the user's shell](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts):
...
The previous examples focused on the content of each message, but note that each element of `input` is a JSON object with `type`, [role](https://platform.openai.com/docs/api-reference/responses/create#responses-create-input), and `content` as follows:
...
<!-- diagram: How the Responses API server derives the prompt from the JSON payload, showing order of system message, tools, instructions, and input items -->
...
<!-- diagram: Expanded prompt showing original items plus reasoning, function_call, and function_call_output appended -->
...
- If the sandbox configuration or approval mode changes, we [insert](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts) a new `role=developer` message with the same format as the original `<permissions instructions>` item.
- If the current working directory changes, we [insert](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts) a new `role=user` message with the same format as the original `<environment_context>`.
...
Our general strategy to avoid running out of context window is to compact the conversation once the number of tokens exceeds some threshold. Specifically, we replace the `input` with a new, smaller list of items that is representative of the conversation, enabling the agent to continue with an understanding of what has happened thus far. An early implementation of [compaction](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts) required the user to manually invoke the `/compact` command, which would query the Responses API using the existing conversation plus [custom instructions for summarization](https://github.com/openai/codex/blob/main/codex-cli/src/prompts/compact.md). Codex used the resulting assistant message containing the summary as the [new input](https://github.com/openai/codex/blob/main/codex-cli/src/agent-loop.ts) for subsequent conversation turns.

Since then, the Responses API has evolved to support a special [/responses/compact endpoint](https://platform.openai.com/docs/api-reference/responses/compact) that performs compaction more efficiently. It returns a [list of items](https://platform.openai.com/docs/api-reference/responses/compact#responses-compact-output) that can be used in place of the previous input to continue the conversation while freeing up the context window. This list includes a special `type=compaction` item with an opaque `encrypted_content` item that preserves the model's latent understanding of the original conversation. Now, Codex automatically uses this endpoint to compact the conversation when the [auto_compact_limit](https://github.com/openai/codex/blob/main/codex-cli/README.md#configuration) is exceeded.
