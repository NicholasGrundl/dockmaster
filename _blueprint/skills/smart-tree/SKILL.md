...
| Recipe   | When                         | Command                                                         |
|----------|----------|-----------------------------------------------------------------|
| **scan** | First look at a path         | `tree -a -L 2 --dirsfirst --noreport --gitignore`               |
| **find** | Looking for a specific topic | `tree -a -f --prune --ignore-case -P "*topic*" --gitignore`     |
| **deep** | Drill into a known directory | `tree -a -L 3 --dirsfirst --noreport --gitignore <dir>`         |
...
```
-a              # show hidden files — critical for config/env discovery
...
|------|---------|-------------|
| `-a` | Show hidden files | Always included in recipes |
...
### If `tree` is missing
If the `tree` command is not available and you cannot install it (e.g., restricted environment), use `find` to reconstruct the structure (hidden files included):

**Scan fallback (depth 2):**
```bash
find <dir> -maxdepth 2 -type d | sort && echo "---FILES---" && find <dir> -maxdepth 2 -type f | sort
```

**Deep fallback (depth 3):**
```bash
find <dir> -maxdepth 3 -type d | sort && echo "---FILES---" && find <dir> -maxdepth 3 -type f | sort
```

**Note:** When using fallbacks, you still provide the **Annotated tree** output by manually formatting the `find` results into a tree-like list for the user.

### Installation
If you have permissions, suggest installation:
