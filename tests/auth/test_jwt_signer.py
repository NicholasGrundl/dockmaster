"""Tests for ServiceUser (JWT signing)."""
...
class TestServiceUserFromDict:
    """ServiceUser initialized from a parsed dict."""
    ...
    def test_creates_from_file_path(self, fake_sa_key_path):
        ...
        su = ServiceUser(credentials=str(fake_sa_key_path))
        assert su.client_email == "test-sa@test-project.iam.gserviceaccount.com"


class TestGetToken:
    """ServiceUser.get_token() produces valid JWTs."""

    def test_returns_string(self, fake_sa_key_data, rsa_public_key_pem):
        from dockmaster.auth.jwt_signer import ServiceUser

        su = ServiceUser(credentials=fake_sa_key_data)
        token = su.get_token(subject="user@example.com", service_name="my-service")
    ...
    def test_claims_aud_is_service_name(self, fake_sa_key_data, rsa_public_key_pem):
        from dockmaster.auth.jwt_signer import ServiceUser
    ...
    def test_claims_iat_and_exp(self, fake_sa_key_data, rsa_public_key_pem):
        ...
        token = su.get_token(subject="user@example.com", service_name="my-service", expiry=3600)
        after = int(time.time())
        claims = jwt.decode(token, rsa_public_key_pem, algorithms=["RS256"], options={"verify_aud": False})
    ...
    def test_default_subject_is_client_email(self, fake_sa_key_data, rsa_public_key_pem):
        ...
        su = ServiceUser(credentials=fake_sa_key_data)
        token = su.get_token(subject=None, service_name="my-service")
        claims = jwt.decode(token, rsa_public_key_pem, algorithms=["RS256"], options={"verify_aud": False})
    ...
    def test_extra_payload_merged(self, fake_sa_key_data, rsa_public_key_pem):
        ...
        su = ServiceUser(credentials=fake_sa_key_data)
        token = su.get_token(subject="u@ex.com", service_name="svc", payload={"role": "admin"})
        claims = jwt.decode(token, rsa_public_key_pem, algorithms=["RS256"], options={"verify_aud": False})
...
class TestGetAuthorization:
    """ServiceUser.get_authorization() returns a Bearer string."""

    def test_returns_bearer_prefix(self, fake_sa_key_data):
        from dockmaster.auth.jwt_signer import ServiceUser

        su = ServiceUser(credentials=fake_sa_key_data)
        auth = su.get_authorization(subject="u@ex.com", service_name="svc")
