...
# ---------------------------------------------------------------------------
# Helper to patch httpx calls
# ---------------------------------------------------------------------------


def _patch_httpx(token_response=None, token_status=200, userinfo_response=None, userinfo_status=200):
...
class TestRefreshHappyPath:
    ...
    def test_valid_refresh_returns_dockmaster_jwt(self, refresh_client, mock_signer):
        mock_client = _patch_httpx()
        with patch("dockmaster.routes.refresh.httpx.AsyncClient", return_value=mock_client):
    ...
    def test_signer_called_with_correct_args(self, refresh_client, mock_signer):
        mock_client = _patch_httpx()
        with patch("dockmaster.routes.refresh.httpx.AsyncClient", return_value=mock_client):
            refresh_client.post(
                "/auth/refresh",
    ...
    def test_service_defaults_to_id_token_audience(self, refresh_client):
    ...
    def test_google_token_called_with_form_data(self, refresh_client):
...
class TestRefreshClientIdResolution:
    ...
    def test_short_client_id_gets_suffix_appended(self, refresh_client, mock_secrets_storage):
...
class TestRefreshGoogleErrors:
    ...
    def test_google_refresh_non_200_returns_401(self, refresh_client):
        """Google token endpoint returns non-200 → 401."""
        mock_client = _patch_httpx(token_status=401, token_response={"error": "invalid_grant"})
        with patch("dockmaster.routes.refresh.httpx.AsyncClient", return_value=mock_client):
            response = refresh_client.post(
                "/auth/refresh",
    ...
    def test_id_token_verification_failure_returns_401(self, refresh_client, mock_realm):
...
class TestRefreshCanIssue:
    ...
    def test_issuer_not_allowed_returns_403(self, refresh_client, mock_realm):
    ...
    def test_audience_not_allowed_returns_403(self, refresh_client, mock_realm):
    ...
    def test_domain_not_allowed_returns_403(self, refresh_client, mock_realm):
...
class TestRefreshUserInfoFailure:
    ...
    def test_userinfo_failure_returns_empty_claims(self, refresh_client):
        """UserInfo non-200 → warning logged, empty profile claims, still succeeds."""
        mock_client = _patch_httpx(userinfo_status=500, userinfo_response={"error": "server error"})
        with patch("dockmaster.routes.refresh.httpx.AsyncClient", return_value=mock_client):
            response = refresh_client.post(
                "/auth/refresh",
                json={"refresh_token": "test-refresh-token"},
