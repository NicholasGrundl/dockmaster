...
class TestGrantList:
    def test_lists_services(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
...
class TestGrantGet:
    def test_gets_grants(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
...
class TestGrantAdd:
    def test_adds_to_existing_service(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
    ...
    def test_creates_new_service_on_404(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.side_effect = [
    ...
    def test_merges_roles_for_existing_subject(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.side_effect = [
            _mock_response(
...
class TestGrantRemove:
    def test_removes_specific_role(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.side_effect = [
            _mock_response(
    ...
    def test_removes_all_roles_without_flag(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.side_effect = [
            _mock_response(
...
class TestGrantDelete:
    def test_deletes_service(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
