...
class TestRoleList:
    def test_lists_roles(self, mocker):
...
class TestRoleGet:
    def test_gets_role(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.return_value = _mock_response(json_data={"name": "admin", "permissions": ["read", "write"]})
...
class TestRoleAdd:
    def test_adds_permission(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
...
class TestRoleRemove:
    def test_removes_permission(self, mocker):
        mock_cls = mocker.patch("dockmaster.cli.http.httpx.Client")
        mock_client = mock_cls.return_value.__enter__.return_value
        mock_client.request.side_effect = [
