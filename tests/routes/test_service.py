"""Tests for POST /auth/service/token — exchange Google credentials for a dockmaster JWT."""
...
class TestServiceTokenJWTPath:
    ...
    def test_custom_expiry(self, exchange_client, signer):
        ...
        response = exchange_client.post(
            "/auth/service/token?expiry=1800",
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_expiry_clamped_to_max(self, exchange_client, signer):
        ...
        response = exchange_client.post(
            "/auth/service/token?expiry=86400",
            headers={"Authorization": f"Bearer {token}"},
        )
...
class TestServiceTokenAccessTokenPath:
    ...
    def test_valid_access_token(self, mocker, app, fake_realm, fake_sa_key_data):
        ...
        with _ExchangeTestClient(app, settings, fake_realm) as client:
            response = client.post(
                "/auth/service/token?service=my-service",
...
class TestServiceTokenErrors:
    """Error cases for /auth/service/token."""

    def test_missing_bearer_token(self, exchange_client):
        """No Authorization header -> 401."""
        response = exchange_client.post("/auth/service/token")
        assert response.status_code == 401
